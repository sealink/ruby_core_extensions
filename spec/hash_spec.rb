...
describe Hash do
  ...
  it "should allow converting all values to strings recursively" do
    @hash1.stringify_values_recursively.should == {:a => "1", :b => {:c => "2"}, :d => "test", :e => ["3", '4.0', "2000-01-01", '2000-01-01T00:00:00+00:00', {:f => "5"}]}
  end
  ...
  it "should allow converting keys" do
    @hash1.convert_keys(&:to_s).should == {"a" => 1, "b" => {:c => 2}, "d" => "test", "e" => @sub_array1}
  end

  it "should allow converting values" do
    @hash1.convert_values(&:to_s).should == {:a => "1", :b => {:c => 2}, :d => "test", :e => @sub_array1}
  end
  
  it "should allow converting values only for specific keys" do
    @hash1.convert_values(:d, :e, &:to_s).should == {:a => 1, :b => {:c => 2}, :d => "test", :e => @sub_array1}
  end

  it "should allow making indifferent access recursively" do
    @hash1.make_indifferent_access_recursively['b']['c'].should eql 2
    @hash1.make_indifferent_access_recursively['e'][4]['f'].should eql 5
  end
